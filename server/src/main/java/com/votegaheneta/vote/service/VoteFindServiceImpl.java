...
public class VoteFindServiceImpl implements VoteFindService {
  ...
  @Transactional(readOnly = true)
  @Override
  public List<VoteInfoDto> findSearchCandidates(Long sessionId, Long voteId, String keyword, Pageable pageable) {
    StringBuilder sb = new StringBuilder();
    keyword = keyword.trim();
    for (String word : keyword.split("")) {
      sb.append(searchComponent.searchWordReSetting(word));
    }
    return customCandidateRepository.findSearchCandidatesBySessionId(sessionId, voteId, sb.toString(), pageable);
  }
